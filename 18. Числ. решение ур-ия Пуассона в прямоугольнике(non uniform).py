...
X, Y = np.meshgrid(x, y) 	#двумерная матрица узлов сетки (X,Y) 
surf = ax.plot_surface(X, Y, u_new, cmap=plt.cm.rainbow, rstride=5, cstride=5)
ax.contourf(X, Y, u_new, zdir='z', offset=0, cmap=plt.cm.rainbow)
fig.colorbar(surf)
plt.show()
